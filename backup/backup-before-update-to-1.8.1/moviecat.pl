...
var scroll = new Scroller(\'MTABLE\', {area: 100, velocity: 1});
$(\'MTABLE\').addEvent(\'mouseover\', scroll.start.bind(scroll));
$(\'MTABLE\').addEvent(\'mouseout\', scroll.stop.bind(scroll));
}); 
</script>
	';
    print_html "</head>";
    print_html '<body><div class="gradient">';
}
...
sub format_movie
{
    ...
    if ($m->img) {
        ...
        print_html "<td class=poster rowspan=4 width=95>".
            "<a href=http://www.imdb.com/title/tt", $m->id, "><img src=\"", $img_link, "\" /></a></td>";
} else {
        my $img_file = img_name($m->id);
        my $img_link = $image_dir ."/". $img_file;
        if ( ! -e $image_cache ."/". $img_file ) { $img_link = $m->img; }
...
#            '<img src="noposter.png"><span style="display:none">?</span></td>';
    ...
    print_html "<a class=MTITLE href=http://www.imdb.com/title/tt",
               $m->id, ">", $m->title, "</a> <span class=vuosi>(<span class=MYEAR>", $m->year, "</span>)</span></b>";
 #   print_html " <small><i>", $m->type, "</i></small>";
    ...
    print_html "<span class=movieheadmeta>Year: <b>", $m->year, "</b>, <span class=ratee>Rating: <b class=MRATING>", $m->user_rating, "</b> &nbsp;&nbsp; </span>",
            "Runtime: <b class=MRUNTIME>", $runtime ? $runtime : "?" , "</b> min",
            " &nbsp;&nbsp; <i class=MGENRE>(", join(' / ',@{$m->genres}), ")</i></a>";
    # user votes
    my $found_vote = 0;
    my $uid;
    for my $uv (@user_vote) {
...
sub print_page_genre
{
    ...
    if (!%{$pmlist}) {
        print_html "<br><br><ul><i>* Ei leffoja *</i></ul><br>&nbsp;&nbsp;<br>";
    } else {
        print_html "Genre: <ul>";
