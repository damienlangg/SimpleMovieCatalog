...
sub cache_image
{
    ...
    my $image = get($m->img);
    if (!$image) {
        print_err "Error getting image: ", $m->img, "\n";
        return 0;
    }
    if (open F_IMG, ">:raw", $img_file) {
        ...
        print_err "Error saving image: ", $img_file, "\n";
        return 0;
    }
    return 1;
}
...
sub open_bom
{
    ...
    if ($bom eq "\x{fe}\x{ff}" or $bom eq "\x{ff}\x{fe}") {
        # print_detail "UNICODE UTF-16: $fname\n";
        seek $FH, 0, 'SEEK_SET';
...
sub process_nfo
{
    ...
    while (<$F_NFO>) {
        if (/imdb\.com\/title\/tt(\d+)/i) {
            ...
            if ( $num_loc > 1 ) { print_detail "*$num_loc"; }
            print_detail " OK\n";
        }
    }
    close $F_NFO;
    if (!$found) {
        # if ( ! -e "imdb.nfo" ) {
        print_log "*** Error: imdb id not found in .nfo: $File::Find::name\n";
        print_detail "$shortname: IMDB NOT FOUND\n";
        # $nfo_no_id++;
...
sub inherit
{
    ...
    print_detail shorten("$dir/"), ": Inherit\n";
...
sub automatch
{
    ...
    if ($title and $year) {
        print_detail shorten($path ."/"), ": GUESS\n";
        print_detail shorten(" ??? Guess: '".$title."' (".$year.")"), ": ";
        my $msearch = findmovie($title, $year);
        if ($msearch and $msearch->id and $msearch->direct_hit) {
            print_detail $msearch->id, " MATCH\n";
            ...
            print_detail "Matches: $nmatch\n";
            $all_dirs{$path}->{matches} = $nmatch;
        } else {
            print_detail "No Match\n";
        }
    }
}
...
=cut
    ...
    #print_detail "Perfect matches: $perf_match\n";
}
...
sub page_start
{
    ...
    open $F_HTML, ">", $fname  or  die "ERROR: can't write $fname\n";
    print_detail "Writing $fname\n";
    html_start;
...
sub list_files
{
    my $dir = shift;
    my @flist;
    opendir(my $dh, $dir) or print_err("ERROR: opendir $dir\n") and return;
    for my $f (readdir($dh)) {
...
sub do_search
{
    ...
    if ($mfound eq undef) {
        ...
        if ($sel >=1 and $sel <= $num_m) {
            ...
            my $m = getmovie($ma->{id});
            if (!$m) {
                print_err "ERROR Getting full info for [", $ma->{id}, "]\n";
            } else {
                print_info " OK.\n";
                save_movie($dir, $m);
...
sub required_arg {
    my ($opt, $arg) = @_;
    if (!$arg) { die "ERROR: Missing arg for $opt\n"; }
    return 1;
}
...
sub makedir
{
    ...
    -d $dir or mkdir $dir or die "ERROR: can't mkdir $dir";
}
...
sub init
{
    ...
    if (!$base_name) {
        die "ERROR: missing file name in base path: $base_path";
    }
